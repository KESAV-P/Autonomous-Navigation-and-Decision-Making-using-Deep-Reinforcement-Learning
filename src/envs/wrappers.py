...
class RewardShapingWrapper(gym.Wrapper):
    ...
    def _get_front_cell(self):
        ...
        grid = getattr(unwrapped, 'grid', None)
        if front_pos is not None and grid is not None:
    ...
    def step(self, action):
        ...
        door_open_before = (front_before is not None and front_before.type == 'door' and front_before.is_open)
        ...
        if not self.key_picked and not carrying_before and carrying_after:
            shaped_reward += self.key_bonus
            self.key_picked = True

        # 2. One-time door open bonus (+0.3)
        front_after = self._get_front_cell()
        door_open_after = (front_after is not None and front_after.type == 'door' and front_after.is_open)
